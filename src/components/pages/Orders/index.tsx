...
const Orders = () => {
  ...
  const fetch = async(pageReceived: number) => {
    try {
      const { status, body } = await getordersAPI(`?page=${pageReceived}&q=${searchTerm}`)
      if (status === 200) {
  ...
  return (
    <div className={`${styles.background}`}>
      <div className={`d-flex justify-content-between`}>
        ...
        <div>
          <SearchTextField placeholder='Search...' name='search_text' type='text' onKeyPress={onKeyPress}  />
        </div>
      </div>
      <div>
        <OrdersTable orders={orders} page={page} total={total} handleChangePage={handleChangePage}  />
      </div>
    </div>
  )
}
